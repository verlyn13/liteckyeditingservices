...
const matches = [];
for (;;) {
	const m = scriptSrcRegex.exec(html);
	if (m === null) break;
	matches.push(m[1]);
}

const decapRefs = matches.filter((src) => /decap-cms(\.min)?\.js/i.test(src));
const npmCmsRefs = matches.filter((src) =>
	/\/admin\/cms(\.[a-f0-9]{8})?\.js/i.test(src),
);
...
if (npmCmsRefs.length === 1 && decapRefs.length === 0) {
	...
	console.error(
		"❌ No admin CMS bundle referenced (expected /admin/cms.js or /admin/cms.<hash>.js).",
	);
	hasError = true;
} else {
	console.error(
		"❌ Found legacy decap-cms references. Admin must use /admin/cms.js only:",
	);
