...
const requiredFiles = JSON.parse(
	fs.readFileSync(
		join(rootDir, "desired-state/repo.required-files.json"),
		"utf8",
	),
);
...
if (fs.existsSync(astroConfigPath)) {
	...
	if (!astroConfig.includes("@tailwindcss/vite")) {
		console.error(
			"❌ astro.config.mjs missing @tailwindcss/vite plugin for Tailwind v4",
		);
		hasErrors = true;
	} else {
...
if (fs.existsSync(prettierPath)) {
	try {
		...
		if (prettier.tailwindStylesheet !== "./src/styles/global.css") {
			console.error(
				"❌ .prettierrc.json tailwindStylesheet must point to ./src/styles/global.css",
			);
			hasErrors = true;
		} else {
...
if (fs.existsSync(globalCssPath)) {
	...
	if (!css.includes('@import "tailwindcss"')) {
		console.error(
			'❌ global.css must import Tailwind v4: @import "tailwindcss"',
		);
		hasErrors = true;
	}
