...
if (packageJson.dependencies?.tailwindcss) {
	...
	if (tailwindSpec === "latest" || /\b4\./.test(tailwindSpec)) {
		...
		console.error(
			`❌ Tailwind CSS should target v4 (or 'latest'), got: ${tailwindSpec}`,
		);
		hasErrors = true;
	}
} else {
...
if (packageJson.dependencies?.["@astrojs/vercel"]) {
	console.error(
		"❌ Found @astrojs/vercel - should not be present (deploying to Cloudflare)",
	);
	hasErrors = true;
}
...
if (packageJson.dependencies?.["@astrojs/netlify"]) {
	console.error(
		"❌ Found @astrojs/netlify - should not be present (deploying to Cloudflare)",
	);
	hasErrors = true;
}
...
console.log(
	"ℹ️  Deployment adapter: keep Cloudflare; no non-CF adapters allowed.",
);
