...
html = html.replace(/\/admin\/cms(?:\.[a-z0-9]+)?\.js/g, `/admin/${hashedName}`);
...
console.log('[cms-hash] Updated admin/index.html to', hashedName, dsn ? '(with sentry-dsn meta)' : '');

// Ensure headers include immutable cache for hashed file
const headersPath = path.join(root, 'public', '_headers');
let headers = fs.existsSync(headersPath) ? fs.readFileSync(headersPath, 'utf8') : '';
