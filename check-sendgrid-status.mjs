      ...
      if (res.statusCode === 200) {
        if (result.messages && result.messages.length > 0) {
          ...
          result.messages.forEach((msg, index) => {
            ...
            console.log(`   Time: ${new Date(msg.last_event_time * 1000).toLocaleString()}`);
          ...
          console.log('3. Messages might be in sandbox mode (not actually sent)');
        }
        ...
        console.log('');
        console.log('Alternative: Check Activity Feed in SendGrid Dashboard directly');
    ...
    res.on('end', () => {
      try {
        ...
        if (result.length > 0) {
          ...
          result.forEach(item => {
            console.log(`   - ${item.email} (${new Date(item.created * 1000).toLocaleDateString()})`);
          });
        } else {
