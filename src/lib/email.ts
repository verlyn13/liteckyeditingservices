...
export async function sendEmail(
  ...
  try {
    ...
    const fromEmail = message.useEmDomain && message.from.includes('@liteckyeditingservices.com')
    ...
    // Mail settings
    const sandboxEnabled = options.devSandbox ?? (import.meta.env.DEV && !import.meta.env.SENDGRID_FORCE_SEND);
    mail.mailSettings = {
      sandboxMode: {
 ...
 */
export function validateContent(message: string): { valid: boolean; reason?: string } {
