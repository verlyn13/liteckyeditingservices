...
export const POST: APIRoute = async ({ request, clientAddress }) => {
  try {
    ...
    const emailRateLimit = checkRateLimit(`email:${email.toLowerCase()}`, 2, 3600000);
    if (!emailRateLimit.allowed) {
      ...
      return new Response(
        ...
        }),
        {
          ...
          headers: {
            'Content-Type': 'application/json',
            'Retry-After': String(Math.ceil((emailRateLimit.resetAt - Date.now()) / 1000))
          }
        }
      );
    }
    ...
    if (!contentValidation.valid) {
      console.warn(`Content validation failed: ${contentValidation.reason} for ${email}`);
      return new Response(
        JSON.stringify({
          success: false,
    ...
    const emailFrom = import.meta.env.EMAIL_FROM || 'hello@liteckyeditingservices.com';
    const emailTo = import.meta.env.EMAIL_TO || 'admin@liteckyeditingservices.com';
    ...
    if (!sendGridApiKey) {
      ...
      const adminResult = await sendEmail(
        ...
        {
          devSandbox: import.meta.env.DEV && !import.meta.env.SENDGRID_FORCE_SEND,
      ...
      if (!adminResult.success) {
        ...
        if (!import.meta.env.DEV) {
          return new Response(
            JSON.stringify({
              success: false,
              error: 'Failed to process your submission. Please try again or email us directly.',
            }),
            { status: 500, headers: { 'Content-Type': 'application/json' } }
          );
        }
      ...
      console.log('Contact form processed successfully:', {
        ...
        sandboxMode: import.meta.env.DEV && !import.meta.env.SENDGRID_FORCE_SEND,
        timestamp: new Date().toISOString(),
      });
    }
