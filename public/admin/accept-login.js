...
(() => {
	...
	function persistUser(token) {
		if (!token || typeof token !== "string")
			throw new Error("persistUser: empty token");
		const user = {
			token,
			backendName: "github",
			login: "github",
		...
		USER_KEYS.forEach((k) => localStorage.setItem(k, raw));
	...
	async function acceptAndFlipFromToken({ token, state = "pkce" }) {
		...
		try {
			...
			log("DIAG", {
				url: location.href,
				ls_user: USER_KEYS.reduce(
					(o, k) => ((o[k] = localStorage.getItem(k)), o),
					{},
				),
				versions: (() => {
					const lines = [];
	...
	window.addEventListener(
		...
		(ev) => {
			...
			if (
				...
				ev.data.startsWith("authorization:github:success:")
			) {
