...
(() => {
  try {
      ...
      const logWrite = (kind, k, v) => {
        try {
          if (/(cms|netlify).*auth.*state/i.test(String(k))) {
            console.log('[ADMIN DEBUG] STORAGE WRITE', { kind, key: k, value: v });
          }
        } catch {}
      };
    ...
    function sweepStates(tag) {
      ...
      const all = { ...take(localStorage, 'LS'), ...take(sessionStorage, 'SS') };
      ...
      (event) => {
        ...
        if (typeof event.data === 'string' && event.data.startsWith('authorization:github:success:')) {
          ...
          try {
            ...
            if (expected == null) {
              expected = Object.values(sweep).find((v) => v && /^[0-9a-f-]{8,}$/i.test(String(v))) ?? null;
            }
    ...
    window.__forceAccept = () => {
      try {
        ...
        for (const k of candidateKeys) {
          try { localStorage.setItem(k, state); } catch {}
          try { sessionStorage.setItem(k, state); } catch {}
        }
