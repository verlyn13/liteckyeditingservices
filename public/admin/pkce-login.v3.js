...
(() => {
  ...
  async function startLogin() {
    ...
    const win = openFn.call(window, u.toString(), 'decap-oauth', 'popup,width=600,height=800');
    try {
  ...
  function neuterDecapAuthorizer() {
    ...
    if (!CMS) return;
    try {
      const authz = CMS?.auth?.authorizer || CMS?.OAuthAuthorizer?.prototype || null;
      if (authz && !authz.__patched) {
        const deny = () =>
          Promise.reject(new Error('Decap authorizer disabled (PKCE externalized).'));
  ...
  async function onPkceSuccess(finalToken, _state) {
    ...
    if (store) {
      try {
        const tryDispatch = (type) => store.dispatch({ type, payload: userObj });
        try {
