...
(() => {
	...
	const bc = (message, data = {}) => {
		...
		try {
			console.info("[HYDR]", message, data);
		} catch {}
	};
	const cap = (err, ctx) => {
		try {
			if (err && !err.name) err.name = "HydratorError";
			getSentry()?.captureException?.(err, { extra: ctx || {} });
		} catch {}
		try {
			console.error("[HYDR] ERROR:", err, ctx);
		} catch {}
	};
	...
	const readUserFromLS = () => {
		for (const k of LS_KEYS) {
			...
			try {
				...
				if (u && typeof u.token === "string" && u.token)
					return { key: k, user: u };
			} catch (e) {
				cap(e, { phase: "parse-ls", key: k });
			}
		}
		return null;
	};
	...
	const reEmitCanonical = (user) => {
		try {
			...
			bc("hydr:re-emit-canonical", {
				state,
				tokenPreview: user.token.slice(0, 6) + "…",
			});
		} catch (e) {
			cap(e, { phase: "re-emit" });
		}
	...
	const manualDispatch = (store, user) => {
		try {
			...
			store.dispatch?.({
				...
				payload: {
					token: user.token,
					backendName: user.backendName || "github",
					login: user.login || "github",
					isFetching: false,
				},
			});
			bc("hydr:manual-dispatch", {
				actions: ["OAUTH_AUTHORIZE_SUCCESS", "LOGIN_SUCCESS"],
			});
		} catch (e) {
			cap(e, { phase: "dispatch" });
		}
	};

	const getStore = () =>
		(window.CMS &&
			(window.CMS.reduxStore || window.CMS.store || window.CMS.getStore?.())) ||
		null;
	const getToken = async () =>
		window.CMS?.getToken ? window.CMS.getToken() : null;
	...
	bc("hydr:ls-user-found", {
		key: userEntry.key,
		login: userEntry.user.login || "github",
	});
	...
	const waitForStore = (resolve, reject) => {
		...
		if (store) {
			bc("hydr:store-available", { attempts });
			resolve(store);
			return;
		}
		if (attempts > 12) {
			cap(new Error("Decap store never appeared"), {
				phase: "wait-store",
				attempts,
			});
	...
	new Promise(waitForStore)
		.then(async (store) => {
			...
			try {
				...
				if (tok) {
					bc("hydr:already-hydrated", {
						tokenPreview: String(tok).slice(0, 6) + "…",
					});
					return;
				}
			} catch (e) {
				cap(e, { phase: "probe-token" });
			}
			...
			setTimeout(async () => {
				...
				if (tok2) {
					bc("hydr:success-after-reemit", {
						tokenPreview: String(tok2).slice(0, 6) + "…",
					});
					return;
				}
				...
				setTimeout(async () => {
					...
					if (tok3) {
						bc("hydr:success-after-dispatch", {
							tokenPreview: String(tok3).slice(0, 6) + "…",
						});
						return;
					}
					...
					try {
						...
						cap(e, { phase: "final-reload" });
					}
				}, 400);
			}, 300);
		})
		.catch((e) => {
			cap(e, { phase: "hydr-root" });
		});
})();
