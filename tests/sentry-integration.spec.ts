...
test.describe('Sentry Integration', () => {
  ...
  test('should capture exceptions', async ({ page }) => {
    ...
    await page.evaluate(() => {
      const w = window as unknown as AdminWindow;
      const sentry = w.__sentry as { captureException?: (e: unknown) => void } | undefined;
      if (sentry) {
...
test.describe('Sentry Test Page', () => {
  const SKIP_PROD =
    !!process.env.BASE_URL && process.env.BASE_URL.includes('liteckyeditingservices.com');
  ...
  test('should capture exception when clicking test button', async ({ page }) => {
    ...
    expect(consoleMessages.some((msg) => msg.includes('[Test] Capturing exception'))).toBe(true);
  });

  test('should create custom span when clicking test button', async ({ page }) => {
    const consoleMessages: string[] = [];
    page.on('console', (msg) => consoleMessages.push(msg.text()));
    ...
    expect(consoleMessages.some((msg) => msg.includes('[Test] Creating custom span'))).toBe(true);
  });

  test('should log info message when clicking test button', async ({ page }) => {
    const consoleMessages: string[] = [];
    page.on('console', (msg) => consoleMessages.push(msg.text()));
    ...
    // Verify log was sent
    expect(consoleMessages.some((msg) => msg.includes('[Test] Info log'))).toBe(true);
  });

  test('should set user context when clicking test button', async ({ page }) => {
    await page.goto(`${BASE_URL}/test-sentry`);
