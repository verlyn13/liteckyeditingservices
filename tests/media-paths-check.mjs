...
if (!existsSync(mediaDir)) fail(`'${mediaDir}' not found. Create this directory or adjust media_folder.`);
...
if (!/media_folder:\s*"public\/uploads"/.test(adminCfg)) fail('functions/admin/config.yml.ts missing media_folder: "public/uploads"');
if (!/public_folder:\s*"\/uploads"/.test(adminCfg)) fail('functions/admin/config.yml.ts missing public_folder: "/uploads"');
ok('functions/admin/config.yml.ts media/public folders OK');

const apiCfg = readFileSync('functions/api/config.yml.ts', 'utf8');
if (!/media_folder:\s*"public\/uploads"/.test(apiCfg)) fail('functions/api/config.yml.ts missing media_folder: "public/uploads"');
if (!/public_folder:\s*"\/uploads"/.test(apiCfg)) fail('functions/api/config.yml.ts missing public_folder: "/uploads"');
