...
async function testSendGridAPI() {
	try {
		...
		console.log(
			`   Response: ${response[0].body || "Email sent successfully"}`,
		);
		...
		if (error.message.includes("401")) {
			...
			console.error(
				"   Check that the SendGrid API key is correct and has proper permissions.",
			);
			...
			console.error(
				"   Make sure hello@liteckyeditingservices.com is a verified sender in SendGrid.",
			);
		}
