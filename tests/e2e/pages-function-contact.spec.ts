...
test.describe("Pages Function /api/contact", () => {
	test.skip(
		!process.env.PAGES_BASE_URL && !process.env.CI,
		"Requires Cloudflare Pages dev server",
	);

	test("responds to POST with JSON", async ({ request, baseURL }) => {
		const target = process.env.PAGES_BASE_URL
			? `${process.env.PAGES_BASE_URL}/api/contact`
			: `${baseURL}/api/contact`;
